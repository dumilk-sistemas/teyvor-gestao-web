                ...
                <View style={productStyles.table}>
                  ...
                  {filteredRows.map((row: any) => {
                    ...
                    return <View key={String(row.id)} style={productStyles.tableRow}>
                      ...
                      {visibleColumns.code && <Text numberOfLines={1} ellipsizeMode="middle" selectable style={[productStyles.cellText, productStyles.smallColumn, productStyles.codeCell]}>{row.code || '—'}</Text>}
                      {visibleColumns.category && <Text numberOfLines={1} style={[productStyles.cellText, productStyles.mediumColumn]}>{row.category || 'Sem categoria'}</Text>}
                      {visibleColumns.unit && <Text numberOfLines={1} style={[productStyles.cellText, productStyles.tinyColumn]}>{row.sale_mode === 'weight' ? 'kg' : row.unit}</Text>}
...
const productStyles = StyleSheet.create({
  ...
  tinyColumn: { width: 42 }, smallColumn: { width: 88 }, mediumColumn: { width: 112 }, codeCell: { paddingRight: 10 }, valueColumn: { width: 88, textAlign: 'right', paddingRight: 10 }, stockColumn: { width: 70, textAlign: 'right', paddingRight: 10 }, statusColumn: { width: 76, alignItems: 'center' }, actionColumn: { width: 66, alignItems: 'flex-end' },
